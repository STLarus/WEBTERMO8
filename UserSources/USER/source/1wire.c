 ...
 */
UINT16 test_alarm(float temp, INT8 highlimit, INT8 lowlimit, UINT8 hist,
UINT8 *state, UINT8 snum)
    {
	...
	if (fTemp <= (fHlimit - hist))
	    {
	    *state = NO_ALARM;
	    NVRAM_Write32(NV_DELAY_SEN1+(snum*4), 0);
	    }
	}
...
//--------------------------------------------------------------------------------
	...
	if (senzor[k].ENBLE)  //ispitivanje alarma
		...
		alarm = test_alarm(temp[k], senzor[k].HIGHLIMIT,
			senzor[k].LOWLIMIT, senzor[k].HIS, &senzor[k].STATE, k);
	    ...
	    if (alarm)
		{
		NVRAM_Write32(NV_DELAY_SEN1+(k*4), 1);
